...
> [!NOTE]
> v1.4.6 是产品化重构后的稳定性修正版：QQ 侧收敛为 `/friday`、`/friup`、`/frihelp`，插件配置改为 AstrBot `_conf_schema.json` 分层对象，QQ/Web 上传统一进入 upload pipeline。未指定分类的图片会进入 `inbox` 系统分类，显示名为“待整理”。本版本加强定时发送结果判断、布尔配置解析、旧目录迁移一致性、WebUI 错误反馈和图库健康检查。
...
| 健康检查 | WebUI 可检查缺失文件、孤儿文件、哈希异常和未知分类 |
