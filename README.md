...
> [!NOTE]
> v1.3 将图库文件迁移为 `library/` 单文件夹存储，分类统一保存为英文 slug，同时保留中文显示名。QQ 侧新增群白名单、上传管理员、定时发图；Web 管理页支持分页、拖拽多图上传和批量操作。v1.3.1 修正发送变换判定：只要 `send_transform=rotate_180`，发送前就会使用旋转副本。
...
| 敏感图变换 | 敏感图默认设置 `rotate_180`，凡 `send_transform=rotate_180` 的图片都会在发送前生成旋转副本，原图不改动 |
